    ...
    <div class="card">
        ...
        <div class="card-body my-1">
            @if ($translates->count() > 0)
                <form id="rick-submited-form" action="{{ route('admin.settings.language.translates.update') }}"
                    method="POST">
                    @csrf
                    @foreach ($translates as $translate)
                        <div class="rick-translate-box">
                            <div class="rick-translated-item d-block d-lg-flex bd-highlight align-items-center">
                                <div class="flex-grow-1 bd-highlight">
                                    <textarea class="rick-translate-key translate-fields form-control" rows="1" readonly>{{ $translate->key }}</textarea>
                                </div>
